...
step_iccv21_09_path = "./process_image/STEP-ICCV21-09/"
...
images = []
for filename in os.listdir(step_iccv21_09_path):
    img = cv2.imread(os.path.join(step_iccv21_09_path, filename))
    images.append(img)
images.reverse()
...
if __name__ == '__main__':
    ...
    while True:
        # ret_val, frame = cam.read()
        if len(images) and detected_frame_num < get_frame_number:  # if next frame
            frame = images.pop()
            origin_frame = frame.copy()
            dets, frame, p0 = detect_track_face(frame)
            if len(dets):
    ...
    for i in range(len(origin_con_frame)):
        ...
        cv2.imwrite(f'./output/step_iccv21_09/recognition/frame_{i}.png', detected_frame_curr)
        good_new = []
        if i == 1:
            ...
            p0 = p1
            cv2.imwrite(f'./output/step_iccv21_09/optical/p0/frame_{i}_{len(p1)}_point.png', p0_frame)
            cv2.imwrite(f'./output/step_iccv21_09/optical/select_good/frame_{i}_{len(good_new)}_point.png',
                        good_new_frame)
